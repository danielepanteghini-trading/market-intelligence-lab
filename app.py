...
st.set_page_config(page_title="Market Intelligence Lab v0.8", layout="wide")
st.title("Market Intelligence Lab v0.8")
...
def evaluate_candidate(frame, family, fast, slow, window, threshold, horizon, split, cost_bps):
    ...
    elif family == "Compression expansion":
        trend = pd.Series(np.where(ema(x.close, fast) > ema(x.close, slow), 1.0, -1.0), index=x.index)
    ...
    elif family == "RSI mean reversion":
        oscillator = rsi(x.close, window)
        long_entry = (oscillator.shift(1) <= threshold) & (oscillator > threshold)
        short_level = 100 - threshold
        short_entry = (oscillator.shift(1) >= short_level) & (oscillator < short_level)
        signal = pd.Series(np.select([long_entry, short_entry], [1.0, -1.0], default=0.0), index=x.index)
    elif family == "Donchian breakout":
        prior_high = x.high.rolling(window).max().shift(1)
        prior_low = x.low.rolling(window).min().shift(1)
        long_entry = (x.close > prior_high) & (x.close.shift(1) <= prior_high.shift(1))
        short_entry = (x.close < prior_low) & (x.close.shift(1) >= prior_low.shift(1))
        signal = pd.Series(np.select([long_entry, short_entry], [1.0, -1.0], default=0.0), index=x.index)
    else:
...
def edge_factory(frame, split, cost_bps):
    ...
    for window, threshold, horizon in product([7, 14, 21], [30, 35, 40], [1, 3, 5]):
        rows.append(evaluate_candidate(frame, "RSI mean reversion", 0, 0, window, threshold, horizon, split, cost_bps))
    for window, horizon in product([10, 20, 40], [1, 3, 5]):
        rows.append(evaluate_candidate(frame, "Donchian breakout", 0, 0, window, 0, horizon, split, cost_bps))
    ...
    for index, row in results.iterrows():
        nearby = results[(results.family == row.family) & (results.horizon_bars == row.horizon_bars)
                         ...
                         & ((results.compression_window - row.compression_window).abs() <= 8)
                         & ((results.threshold - row.threshold).abs() <= 5.01)]
        robust = ((nearby.train_profit_factor > 1) & (nearby.test_profit_factor > 1)
                  & (nearby.train_expectancy_pct > 0) & (nearby.test_expectancy_pct > 0))
        results.loc[index, "plateau_score"] = robust.mean() if len(nearby) else np.nan
    results["candidate"] = ((results.train_events >= 30) & (results.test_events >= 20)
                            ...
                            & (results.train_expectancy_pct > 0) & (results.test_expectancy_pct > 0)
                            & (results.plateau_score >= 0.60))
    positive = ((results.train_profit_factor > 1) & (results.test_profit_factor > 1)
                & (results.train_expectancy_pct > 0) & (results.test_expectancy_pct > 0))
    enough = (results.train_events >= 30) & (results.test_events >= 20)
    results["status"] = np.select(
        [results.candidate, positive & ~enough, positive & enough],
        ["Candidate", "Promising, insufficient sample", "Positive, unstable parameters"],
        default="Rejected",
    )
...
with edge_tab:
    ...
    edge_assets = st.multiselect(
        "Assets to test separately",
        ["SPY", "QQQ", "XLE", "GLD", "TLT", "AAPL", "MSFT", "NVDA", "TSLA"],
        default=[sym],
        max_selections=4,
    )
    if st.button("Run Edge Factory", type="primary", key="edge"):
        try:
            ...
            with st.spinner("Testing candidate hypotheses…"):
                for asset in edge_assets:
                    bars = getbars(asset, days, tf)
                    asset_results = edge_factory(bars, split, cost_bps)
                    asset_results.insert(0, "asset", asset)
                    result_frames.append(asset_results)
                    diagnostics.append({
                        "asset": asset,
                        "bars": len(bars),
                        "from": bars.timestamp.min(),
                        "to": bars.timestamp.max(),
                        "max_train_events": int(asset_results.train_events.max()),
                        "max_test_events": int(asset_results.test_events.max()),
                    })
            results = pd.concat(result_frames, ignore_index=True)
            diagnostic_frame = pd.DataFrame(diagnostics)
            total_bars = int(diagnostic_frame.bars.sum())
            candidates = int(results.candidate.sum())
            promising = int((results.status == "Promising, insufficient sample").sum())
            c1, c2, c3 = st.columns(3)
            c1.metric("Bars loaded", f"{total_bars:,}")
            c2.metric("Preliminary candidates", candidates)
            c3.metric("Promising but insufficient", promising)
            st.dataframe(diagnostic_frame, use_container_width=True, hide_index=True)
            if candidates == 0:
                enough = (results.train_events >= 30) & (results.test_events >= 20)
                if not enough.any():
                    st.info("No candidate: no configuration reached 30 train events and 20 test events. More data or more frequent signals are required.")
                elif promising:
                    st.info("No candidate passed parameter stability. Some positive configurations remain research leads only.")
                else:
                    st.info("No candidate: the tested rules did not remain positive in both train and unseen test after costs.")
...
**Current families:** EMA trend, compression/expansion, RSI mean reversion and Donchian breakout.
...
st.caption("v0.8 • Technical Scanner + Edge Factory • No automatic orders")
